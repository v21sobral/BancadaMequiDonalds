...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) console.warn('Supabase não configurado: defina SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY.');
...
app.get('/mensagens', async (_req, res) => {
  try {
    const data = await supabase('mensagens?select=id,titulo,texto,data_hora,criado_em,autor_id,usuarios:autor_id(id,nome)&order=data_hora.desc,id.desc');
    res.json(data.map((m) => ({
