...
app.get('/health', async (_req, res) => {
  try {
    await supabase('usuarios?select=id&limit=1');
    await supabase('mensagens?select=id&limit=1');
...
if (require.main === module) app.listen(port, () => console.log(`API na porta ${port}`));
module.exports = app;
